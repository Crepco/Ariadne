...
def run_one(ctx: ScoringContext, start: dict, graph_size: int) -> dict:
    ...
    try:
        result = run_agent(start["name"], verbose=False)
        ...
        row.update(
            ...
            matches_baseline=False, optimal=False, tool_calls=0, steps=0, time_seconds=0.0,
            error=str(e),
        )
    return row
